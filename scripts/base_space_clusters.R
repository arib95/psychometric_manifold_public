...
suppressPackageStartupMessages({
  library(mgcv); library(igraph); library(Matrix)
  library(ggplot2); library(dplyr); library(tidyr); library(readr)
  library(scales); library(cluster); library(tibble)
})
...
TOL         <- 1e-5
EPS         <- 1e-10
MIN_POS_DEF <- get0("DX_MIN_POS", ifnotfound = 10L)
MIN_NEG_DEF <- get0("DX_MIN_NEG", ifnotfound = 10L)

# usage/regularisation knobs
MASS_FLOOR_FRAC <- 0.05   # stricter alive threshold (was 0.02)
BALANCE_EVERY   <- 8L
RESEED_MAX      <- 3L
ENTROPY_PUSH    <- 6e-4   # gentler than 1e-3
...
PEN_EMPTY <- 0.60
PEN_IMB   <- 0.15
PEN_RED   <- 0.30   # NEW: redundancy penalty for nearly-duplicate columns of V
...
build_P_from_DxW_A <- function(Base_in, Dx_in,
  ...
  if (!length(keep)) for (th in c(8L,5L,3L,1L)) { cand <- names(which(pos >= th & neg >= th)); if (length(cand)) { keep <- cand; break } }
  if (!length(keep)) {
...
Pi_long <- mem_df %>%
  ...
  tidyr::pivot_longer(dplyr::starts_with("C"), names_to = "cluster", values_to = "pi") %>%
  dplyr::mutate(pi_clip = pmin(pmax(pi, 1e-6), 1-1e-6),
                logit_pi = qlogis(pi_clip))

pred_list <- lapply(unique(Pi_long$cluster), function(cl){
  dfc <- Pi_long[Pi_long$cluster==cl, c("logit_pi","u1","u2")]
  fit <- try(mgcv::bam(logit_pi ~ s(u1, u2, bs="tp", k=80),
  ...
  eta <- as.numeric(predict(fit, newdata = UD$grid, type = "link"))
  p   <- plogis(eta); p[!UD$mask] <- NA
  data.frame(cluster = cl, u1 = UD$grid$u1, u2 = UD$grid$u2, p = p)
})
Fstd_cl <- dplyr::bind_rows(pred_list)

br_fixed <- c(.15,.25,.40,.60,.80)
p_fields <- ggplot(Fstd_cl, aes(u1, u2)) +
  geom_raster(aes(fill = p), interpolate = TRUE) +
  geom_contour(aes(z = p), breaks = br_fixed, colour = "white", linewidth = .28, alpha = .95, na.rm = TRUE) +
  geom_point(data = UD$Uxy, aes(u1, u2), inherit.aes = FALSE, shape = 20, size = 0.28,
             colour = scales::alpha("black", 0.35)) +
  coord_equal(xlim = c(-1,1), ylim = c(-1,1), expand = FALSE) +
  facet_wrap(~ cluster, ncol = 4, scales = "fixed") +
  scale_fill_viridis_c(limits = c(0,1), labels = percent, na.value = NA, name = "π") +
  labs(title = sprintf("Soft cluster fields (K=%d, λ=%.2f)", best_all$K, best_all$lambda),
       x = "u1 (whitened b1,b2)", y = "u2") +
  theme_minimal(11) + theme(panel.grid = element_blank())
ggsave("FIG_softcluster_fields_UNITDISK_Kbest.png", p_fields, width = 7.4, height = 6.2, dpi = 320, bg = "white")

Vb <- best_all$V
colnames(Vb) <- paste0("C", seq_len(ncol(Vb))); rownames(Vb) <- dx
H <- as.data.frame(Vb) |>
...
p_heat <- ggplot(H, aes(cluster, dx, fill = loading)) +
  geom_tile() +
  scale_fill_viridis_c(name = "loading", labels = scales::number_format(accuracy = 0.01)) +
  labs(x = "cluster", y = NULL, title = "Diagnosis archetypes per soft cluster") +
  theme_minimal(11) + theme(panel.grid = element_blank())
ggsave("FIG_cluster_by_dx_heatmap_Kbest.png", p_heat, width = 6.8, height = 5.8, dpi = 320, bg = "white")

print(p_fields); print(p_heat)
cat("Wrote: soft_memberships.csv, cluster_archetypes_dx_by_k.csv, nmf_sweep_metrics.csv, figures.\n")